...
//
// All fields are base64-encoded raw bytes as used by Application mode.
...
func SealRequest(cfgJSON string, method string, path string, headersJSON string, body []byte) ([]byte, error) {
    ...
    msg, err := qtls.SealRequest(ctx, qtls.Options{Provider: prov}, keys, crypto.ModeApplication, method, path, headers, body)
    if err != nil {
        return nil, err
    }
...
func parse(cfgJSON string) (BridgeConfig, qtls.Keys, error) {
    ...
    if err != nil { return BridgeConfig{}, qtls.Keys{}, err }
